...
int main()
{
    ...
    FILE *freq_p = fopen("frequency.txt", "w");
    double gSR_var;
    double time;
    double Iinj = 1.0;
    ...
    for (double i = start_gSR; i <= stop_gSR; i += 0.01)
    {
        ...
        // NEW: initialize the previous value to the first voltage value
        double v_previous = 0;
        double v_current = 0;
        double threshold = -20.0;

        double time_at_first_spike = 0;
        double time_at_last_spike = 0;
        ...
        for (int n = 0; n < max_steps; n++)
        {
            ...
            if (v_previous < threshold && v_current > threshold)
            {
                if (spike_count == 0)
                {
                    time_at_first_spike = time;
                }

                time_at_last_spike = time;

                spike_count++;
            }

            v_previous = v_current;
            //  cout << "previous val: " << v_previous << "/n";
